...
function display(arr) {
    ...
    for (var i = 0; i < arr.length; i++) {
     ...
     <tr>
              ...
              <td><button class="btn btn-danger" id="deleteBtn" onclick="deleteFn(${searchIndex ? searchIndex : i})">Delete</button></td>
...
function searchFn(term) {
    ...
    for (var i = 0; i < itemList.length; i++) {
        if (itemList[i].item_name.includes(term)) {
            itemList[i].newName = itemList[i].item_name
                .toLowerCase()
                .replace(
                    term.toLowerCase(),
...
/*****************************************************fix add item already exist bug ****************************************************************/

function checkExist() {
    var isExist = false;
    for (var i = 0; i < itemList.length; i++) {
        if (
            itemList[i].item_name.trim().toLowerCase() ==
            item_value.value.trim().toLowerCase()
        ) {
            // isExist = true;
